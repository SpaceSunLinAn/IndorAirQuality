 ...
 */
//使用 typedef 定义一个函数指针 
//typedef int8_t (*zmod44xx_i2c_ptr_t) (uint_t addr, uint_t reg_addr, unit_t* data, uint8_t len);

//typedef void (*zmod44xx_i2c_ptr_t)(uint8_t addr, uint8_t reg_addr,
  //                                   uint8_t* data, uint16_t len);
typedef int (*zmod44xx_i2c_ptr_t)(uint8_t addr, uint8_t reg_addr,
                                     uint8_t* data, uint8_t len);
/**
 * @brief function pointer to hardware dependent delay function
 */
typedef void (*zmod44xx_delay_ptr_p)(uint16_t ms);

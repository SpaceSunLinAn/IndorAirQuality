...
int8_t zmod44xx_init_sensor(zmod44xx_dev_t* dev)
{
    ...
    /* prepare sensor */
    dev->read(dev->i2c_addr, 0xB7, data, 1);
   ...
   dev->write(dev->i2c_addr, init_conf->h.addr, data, init_conf->h.len);

    dev->write(dev->i2c_addr, init_conf->d.addr, init_conf->d.data,
                    init_conf->d.len);

    dev->write(dev->i2c_addr, init_conf->m.addr, init_conf->m.data,
                    init_conf->m.len);
  
    dev->write(dev->i2c_addr, init_conf->s.addr, init_conf->s.data,
                    init_conf->s.len);

    dev->write(dev->i2c_addr, ZMOD44XX_ADDR_CMD, &init_conf->start, 1);
    ...
    dev->read(dev->i2c_addr, init_conf->r.addr, data, init_conf->r.len);
    if(ret) {
        return ERROR_I2C;
    }
...
int8_t zmod44xx_init_measurement(zmod44xx_dev_t* dev)
{
    ...
    data[0] = (uint8_t)((uint16_t)hspf >> 8);
    data[1] = (uint8_t)((uint16_t)hspf & 0x00FF);
    dev->write(dev->i2c_addr, meas_conf->h.addr, data, meas_conf->h.len);
   

    dev->write(dev->i2c_addr, meas_conf->d.addr, meas_conf->d.data,
                    meas_conf->d.len);

    dev->write(dev->i2c_addr, meas_conf->m.addr, meas_conf->m.data,
                    meas_conf->m.len);

    dev->write(dev->i2c_addr, meas_conf->s.addr, meas_conf->s.data,
                    meas_conf->s.len);
...
{
    dev->write(dev->i2c_addr, ZMOD44XX_ADDR_CMD, &meas_conf->start, 1);
    return ZMOD44XX_OK;
}
...
int8_t zmod44xx_read_status(zmod44xx_dev_t* dev, uint8_t* status)
{
    ...
    dev->read(dev->i2c_addr, ZMOD44XX_ADDR_STATUS, &st, 1);
  //  if (0 != ret) {
  //      return ret;
  //  }
    *status = st;
...
int8_t zmod44xx_read_rmox(zmod44xx_dev_t* dev, float* rmox)
{
    ...
    dev->read(dev->i2c_addr, meas_conf->r.addr, data, meas_conf->r.len);
